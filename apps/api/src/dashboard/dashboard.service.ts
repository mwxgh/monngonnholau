...
const PAID_STATUSES: OrderStatus[] = ['PAID', 'PROCESSING', 'SHIPPING', 'DELIVERED'];
...
export class DashboardService {
  ...
  async getCustomers() {
    ...
    return Array.from(phoneMap.values()).map((a) => ({
      fullName: a.fullName,
      phone: a.phone,
      email: a.email,
      ...
      lastOrderAt: a.orders.length > 0
        ? a.orders.reduce((latest, o) =>
  ...
  async getAnalytics() {
    const [orders, topItems] = await Promise.all([
      this.prisma.order.findMany({
        select: { status: true, paymentMethod: true, total: true, createdAt: true },
      }),
  ...
  async getStats() {
    ...
    const [revenueAgg, totalOrders, totalProducts, addresses, yearOrders, recentOrders] =
      await Promise.all([
