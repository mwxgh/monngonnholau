...
const PROVINCES_API = ADDRESS_MODE === "old"
  ...
  return (
    <div className="flex flex-col max-h-[90vh]">
      <div className="px-6 pt-6 pb-4 border-b shrink-0">
        <h2 className="text-xl font-bold text-neutral-800">Thanh toán đơn hàng</h2>
        <p className="mt-0.5 text-sm text-neutral-500">
          Quét mã QR để thanh toán — đơn hàng <span className="font-medium">#{info.orderId}</span>
        </p>
      </div>
      ...
      <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col items-center gap-4">
        ...
        <div className="relative">
          <div className={cn("p-3 rounded-xl border bg-white", expired && "opacity-30")}>
            <QRCodeSVG value={info.qrCode} size={208} />
          </div>
          {expired && (
            <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-white/80">
              <p className="text-sm font-semibold text-red-500">QR đã hết hạn</p>
            </div>
          )}
        </div>
        ...
        <div className="text-center">
          <p className="text-2xl font-bold text-primary">{formatVND(info.total)}</p>
          <p className="text-xs text-muted-foreground mt-0.5">Tổng thanh toán</p>
        </div>
        ...
        {!expired && (
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            QR hết hạn sau <span className="font-mono font-semibold text-foreground">{mins}:{secs}</span>
          </div>
        )}
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              const data = await res.json();
              if (data.paymentStatus === "PAID" || data.orderStatus === "PAID") onPaid();
            } catch {
              // ignore
            } finally {
              setChecking(false);
...
export function OrderModal({ open, onClose }: OrderModalProps) {
  ...
  const [cart, setCart] = useState<Record<number, { selectedSku: string; qty: number }>>({});
  ...
  const form = useForm<InfoValues>({
    ...
    defaultValues: { name: "", phone: "", email: "", province: "", district: "", ward: "", street: "", note: "" },
  });
  ...
  function loadProducts() {
    ...
    Promise.all([
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/dashboard/public-settings`)
        .then((r) => r.json())
        .catch(() => ({})),
    ])
      .then(([productData, provinceData, settings]) => {
        ...
        if (settings?.shipping_fee) setShippingFee(Number(settings.shipping_fee));
        const init: Record<number, { selectedSku: string; qty: number }> = {};
        productData.forEach((p) => {
          if (p.variants.length > 0) init[p.id] = { selectedSku: p.variants[0].sku, qty: 0 };
        });
        setCart(init);
      })
  ...
  function setQty(productId: number, delta: number) {
    setCart((prev) => {
      ...
      return { ...prev, [productId]: { ...row, qty: Math.min(Math.max(0, row.qty + delta), max) } };
    });
  }
  ...
  const cartItems = products
    .flatMap((p) => {
      ...
      if (!variant) return [];
      return [{ sku: row.selectedSku, qty: row.qty, price: Number(variant.price), variantName: variant.name, productName: p.name }];
    });
  ...
  async function handleSubmit(values: InfoValues) {
    setServerError("");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/orders/quick`, {
        method: "POST",
      ...
      const data = await res.json();
      if (!res.ok) { setServerError(data.message ?? "Đặt hàng thất bại."); return; }
      setOrderId(data.orderId);
      if (data.qrCode) {
        setPaymentInfo({ qrCode: data.qrCode, checkoutUrl: data.checkoutUrl, orderId: data.orderId, total: data.total });
        setStep(3);
      } else {
        setStep("success");
      }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent showCloseButton className="p-0 overflow-hidden max-w-xl w-full">

        {/* ── Step 1: Chọn sản phẩm ──────────────────────────────────── */}
        {step === 1 && (
          <div className="flex flex-col max-h-[90vh]">
            <div className="px-6 pt-6 pb-4 border-b shrink-0">
              <h2 className="text-xl font-bold text-neutral-800">Đặt hàng nhanh</h2>
              <p className="mt-0.5 text-sm text-neutral-500">Chọn sản phẩm và số lượng bạn muốn đặt</p>
            </div>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-4">
              {loadingProducts ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 gap-3 text-center">
                  <p className="text-sm text-muted-foreground">Không thể tải sản phẩm. Vui lòng thử lại.</p>
                  <button
                    type="button"
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          row.qty > 0 ? "border-primary/40 bg-primary/5" : "border-border bg-card",
                        )}
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="text-right shrink-0">
                            <span className="text-sm font-bold text-primary">{formatVND(price)}</span>
                            {comparePrice && (
                              <span className="text-xs text-muted-foreground line-through ml-1.5">{formatVND(comparePrice)}</span>
                            )}
                          </div>
                        </div>
                        <div className="mt-3 flex items-center gap-3 flex-wrap">
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button type="button" onClick={() => setQty(product.id, -1)} disabled={row.qty === 0}
                            ...
                            <span className="w-8 text-center text-sm font-medium tabular-nums">{row.qty}</span>
                            <button type="button" onClick={() => setQty(product.id, 1)}
            ...
            <div className="px-6 py-4 border-t bg-muted/30 shrink-0 space-y-3">
                ...
                <div className="text-sm space-y-1">
                  {cartItems.map((i) => (
                    <div key={i.sku} className="flex justify-between text-muted-foreground">
                      <span>{i.productName} {i.variantName} × {i.qty}</span>
              ...
              <button type="button" disabled={!hasItems} onClick={() => setStep(2)}
        ...
        {step === 2 && (
          <div className="flex flex-col max-h-[90vh]">
            <div className="px-6 pt-6 pb-4 border-b shrink-0">
              ...
              <h2 className="text-xl font-bold text-neutral-800">Thông tin giao hàng</h2>
              <p className="mt-0.5 text-sm text-neutral-500">Nhập thông tin để chúng tôi giao hàng đến bạn</p>
            </div>
                ...
                <form id="order-form" onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <FormField control={form.control} name="name" render={({ field }) => (
                      <FormItem className="flex-1">
                        <FormLabel className="text-sm font-medium text-neutral-700">Họ tên <span className="text-primary">*</span></FormLabel>
                        <FormControl><AcInput placeholder="Nguyễn Văn A" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={form.control} name="phone" render={({ field }) => (
                      <FormItem className="flex-1">
                        <FormLabel className="text-sm font-medium text-neutral-700">Số điện thoại <span className="text-primary">*</span></FormLabel>
                        <FormControl><AcInput type="tel" placeholder="0912 345 678" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                  </div>
                  ...
                  <FormField control={form.control} name="email" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><AcInput type="email" placeholder="email@example.com (tuỳ chọn)" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name="province" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium text-neutral-700">Tỉnh / Thành phố <span className="text-primary">*</span></FormLabel>
                      <FormControl>
                        <AddressSelect
                  ...
                  {ADDRESS_MODE === "old" && (
                    <FormField control={form.control} name="district" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-sm font-medium text-neutral-700">Quận / Huyện <span className="text-primary">*</span></FormLabel>
                        <FormControl>
                          <AddressSelect
                  ...
                  <FormField control={form.control} name="ward" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium text-neutral-700">
                        {ADDRESS_MODE === "old" ? "Phường / Xã" : "Xã / Phường / Thị trấn"}
                        {" "}<span className="text-primary">*</span>
                      </FormLabel>
                      <FormControl>
                        <AddressSelect
                          placeholder={ADDRESS_MODE === "old" ? "Chọn phường/xã" : "Chọn xã/phường/thị trấn"}
                          options={wards}
                          value={field.value}
                          onChange={(name) => field.onChange(name)}
                          disabled={ADDRESS_MODE === "old" ? !selectedDistrictCode : !selectedProvinceCode}
                  ...
                  <FormField control={form.control} name="street" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium text-neutral-700">Số nhà, tên đường <span className="text-primary">*</span></FormLabel>
                      <FormControl><AcInput placeholder="VD: 12 Nguyễn Huệ" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name="note" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-sm font-medium text-neutral-700">Ghi chú</FormLabel>
                      <FormControl><AcInput placeholder="Ghi chú cho đơn hàng (tuỳ chọn)" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  ...
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-neutral-700">Phương thức thanh toán <span className="text-primary">*</span></p>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      >
                        <CreditCard className={cn("w-5 h-5 shrink-0", paymentMethod === "online" ? "text-primary" : "text-muted-foreground")} />
                        <div>
                          <p className="text-xs font-semibold leading-tight">Thanh toán ngay</p>
                          <p className="text-[11px] text-muted-foreground leading-tight mt-0.5">Chuyển khoản / QR</p>
                        </div>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <Truck className={cn("w-5 h-5 shrink-0", paymentMethod === "cod" ? "text-primary" : "text-muted-foreground")} />
                        <div>
                          <p className="text-xs font-semibold leading-tight">Thanh toán khi nhận</p>
                          <p className="text-[11px] text-muted-foreground leading-tight mt-0.5">COD</p>
                        </div>
                      </button>
                    </div>
                  </div>
            ...
            <div className="px-6 py-4 border-t bg-muted/30 shrink-0">
              <button form="order-form" type="submit" disabled={form.formState.isSubmitting}
        ...
        {step === "success" && (
          <div className="flex flex-col items-center justify-center px-8 py-12 text-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-neutral-800">Đặt hàng thành công!</h2>
              <p className="mt-1 text-sm text-neutral-500">
                Mã đơn hàng: <span className="font-semibold text-neutral-700">#{orderId}</span>
              </p>
            </div>
            <p className="text-sm text-neutral-500 max-w-xs">
              Chúng tôi sẽ liên hệ xác nhận qua số điện thoại bạn đã cung cấp. Cảm ơn bạn đã tin tưởng Món Ngon Nhớ Lâu!
            </p>
