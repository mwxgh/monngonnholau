...
export default function AdminSettingsPage() {
  ...
  async function saveField(key: keyof Settings, value: string) {
    setSaving(key);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/dashboard/settings`, {
        method: "PATCH",
        headers: authHeaders(),
  ...
  async function handleChangePassword() {
    if (pwForm.next !== pwForm.confirm) { setPwMsg({ type: "err", text: "Mật khẩu mới không khớp." }); return; }
    if (pwForm.next.length < 8) { setPwMsg({ type: "err", text: "Mật khẩu phải ít nhất 8 ký tự." }); return; }
    ...
    } catch { setPwMsg({ type: "err", text: "Không thể kết nối máy chủ." }); }
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">Cấu hình thông tin cửa hàng và vận chuyển.</p>
      </div>
      ...
      {[
        {
          ...
          keys: ["store_name", "store_phone", "store_email", "store_website"] as (keyof Settings)[],
        },
        {
          title: "Vận chuyển",
        ...
        <Card key={title}>
          ...
          <CardContent className="space-y-4">
            {FIELD_LABELS.filter((f) => keys.includes(f.key)).map(({ key, label, type }) => {
              ...
              return (
                <div key={key} className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {saving === key ? "..." : saved === key ? "Đã lưu ✓" : "Lưu"}
                    </button>
                  </div>
                </div>
              );
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {pwMsg && (
            <p className={`text-sm ${pwMsg.type === "ok" ? "text-emerald-600" : "text-red-500"}`}>
              {pwMsg.text}
            </p>
          )}
