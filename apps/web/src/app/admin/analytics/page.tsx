...
interface Analytics {
  ...
  topProducts: { name: string; sku: string; totalQty: number; totalRevenue: number }[];
...
export default function AdminAnalyticsPage() {
  ...
  useEffect(() => {
    fetchAdmin(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/dashboard/analytics`)
      .then(async (r) => {
  ...
  if (error || !data) return (
    <p className="text-center text-sm text-red-500 pt-20">{error || "Không thể tải dữ liệu"}</p>
  );
  ...
  const onlineTotal = data.paymentBreakdown.count.ONLINE + data.paymentBreakdown.count.COD;
  const onlinePct = onlineTotal > 0 ? Math.round((data.paymentBreakdown.count.ONLINE / onlineTotal) * 100) : 0;

  return (
    <div className="space-y-6 max-w-7xl">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Phân tích</h1>
        <p className="text-muted-foreground text-sm mt-0.5">Tổng quan chi tiết về đơn hàng và doanh thu.</p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {Object.entries(data.statusBreakdown)
              .sort(([, a], [, b]) => b - a)
              .map(([status, count]) => {
                const meta = STATUS_META[status] ?? { label: status, color: "bg-gray-400" };
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="h-4 rounded-full overflow-hidden flex">
              <div className="bg-sky-400 h-full transition-all" style={{ width: `${onlinePct}%` }} />
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={label} className="rounded-xl border p-4 space-y-1">
                  ...
                  <p className="text-2xl font-bold">{count} <span className="text-sm font-normal text-muted-foreground">({pct}%)</span></p>
                  <p className="text-xs text-muted-foreground">Doanh thu: <span className="font-medium text-foreground">{formatVND(revenue)}</span></p>
                </div>
              ))}
            </div>
          </CardContent>
              ...
              data.topProducts.map((p, i) => (
                <div key={p.sku} className="space-y-1">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold shrink-0">{p.totalQty} <span className="text-muted-foreground font-normal">sp</span></span>
                  </div>
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div
                      className="h-full rounded-full bg-primary/70 transition-all"
                      style={{ width: `${(p.totalQty / maxProductQty) * 100}%` }}
                    />
                  </div>
                  <p className="text-[11px] text-muted-foreground text-right">{formatVND(p.totalRevenue)}</p>
                </div>
              ))
            )}
          </CardContent>
