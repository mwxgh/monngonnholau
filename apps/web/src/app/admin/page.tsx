      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            ...
            <CardDescription>Năm {currentYear} — đơn vị: nghìn ₫</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {monthlyRevenue.map(({ month, revenue }) => (
                <div key={month} className="flex flex-col items-center gap-1.5 flex-1 min-w-0">
                  <div
                    ...
                    style={{ height: `${Math.max((revenue / maxRevenue) * 100, revenue > 0 ? 2 : 0)}%` }}
                ...
                return (
                  <div key={order.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium truncate leading-tight">{order.customerName}</p>
                      <p className="text-[11px] text-muted-foreground truncate">{order.firstItem}</p>
                    </div>
                    <div className="text-right shrink-0 space-y-0.5">
                      <p className="text-xs font-semibold">{formatVND(order.total)}</p>
                      <span className={`inline-block text-[10px] px-1.5 py-0.5 rounded font-medium ${s.className}`}>
                        {s.label}
                      </span>
                    </div>
                  </div>
                ...
                {recentOrders.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-muted/40 transition-colors">
                      ...
                      <td className="py-3 text-muted-foreground text-xs truncate max-w-50">{order.firstItem}</td>
                      <td className="py-3">
