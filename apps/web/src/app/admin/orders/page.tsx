...
const STATUS_META: Record<string, { label: string; className: string }> = {
  PENDING_PAYMENT: { label: "Chờ TT",    className: "text-amber-700 bg-amber-50 border border-amber-200" },
  PAID:            { label: "Đã TT",     className: "text-sky-700 bg-sky-50 border border-sky-200" },
  PROCESSING:      { label: "Chờ xử lý", className: "text-violet-700 bg-violet-50 border border-violet-200" },
  SHIPPING:        { label: "Đang giao", className: "text-blue-700 bg-blue-50 border border-blue-200" },
  DELIVERED:       { label: "Đã giao",   className: "text-emerald-700 bg-emerald-50 border border-emerald-200" },
  CANCELLED:       { label: "Đã hủy",    className: "text-red-700 bg-red-50 border border-red-200" },
  REFUNDED:        { label: "Hoàn tiền", className: "text-gray-700 bg-gray-50 border border-gray-200" },
};
...
export default function AdminOrdersPage() {
  ...
  async function changeStatus(id: number, status: string) {
    setUpdating(id);
    try {
      const res = await fetchAdmin(`${process.env.NEXT_PUBLIC_API_URL}/api/orders/admin/${id}/status`, {
        method: "PATCH",
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">Quản lý toàn bộ đơn hàng của cửa hàng.</p>
      </div>
        ...
        <CardHeader className="flex flex-row items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {ORDER_STATUSES.map(({ value, label }) => (
              ...
              >
                {label}
                {value === "" && <span className="ml-1 font-normal">({orders.length})</span>}
                {value !== "" && (
                  <span className="ml-1 font-normal">({orders.filter((o) => o.status === value).length})</span>
                )}
              </button>
            ))}
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {filtered.map((order) => {
                    const s = STATUS_META[order.status] ?? { label: order.status, className: "" };
                    const nextStatuses = NEXT_STATUSES[order.status] ?? [];
                    return (
                      <tr key={order.id} className="hover:bg-muted/40 transition-colors">
                        <td className="py-3 text-xs font-mono text-muted-foreground">#{order.id}</td>
                        <td className="py-3">
                        ...
                        <td className="py-3 text-xs text-muted-foreground max-w-48">
                          ...
                          {order.items.length > 1 && (
                            <p className="text-[11px] text-muted-foreground/70">+{order.items.length - 1} sản phẩm</p>
                          )}
                        </td>
                        <td className="py-3">
                        ...
                        <td className="py-3 text-right font-semibold text-sm">{formatVND(order.total)}</td>
                        <td className="py-3 text-xs text-muted-foreground whitespace-nowrap">{formatDate(order.createdAt)}</td>
